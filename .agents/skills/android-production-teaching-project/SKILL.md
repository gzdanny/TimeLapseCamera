...
每个项目维护一份 `docs/API_REFERENCE.md`，规划阶段一次性核查所有主要 API，后续增量更新。

**为什么需要清单：**
- 防止"凭记忆写 API"导致的低级错误（已废弃、参数类型错、返回类型不匹配）
- 沉淀项目知识，新人/学生能快速了解每个 API 的坑点
- 审查时有据可查

**清单格式：**

| API | 用途 | 核查结论（含坑点） | 文档链接 |
|-----|------|-------------------|---------|
| 完整 API 名 | 一句话说明用途 | 正确用法 + 常见坑 + 替代方案 | developer.android.com 链接 |

**维护规则：**
1. 规划阶段：列出功能涉及的主要 API → 逐个查文档 → 填入清单 → 再写代码
2. 新增功能：用到新 API 时增量添加
3. 踩坑后：把坑点补充到对应 API 的"核查结论"中
4. 依赖升级：重新核查受影响的 API
